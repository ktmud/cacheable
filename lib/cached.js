...
Cached.prototype.DEFAULT_KEY =  '{_model_}:{_fn_}:{0}'
...
Cached.prototype.wrap = function(fn, key, ttl, ctx) {
  ...
  if (uniqkey in allkeys) {
    console.error('[cached] Possible key conflict: %s', uniqkey)
    console.trace()
  } else {
...
Cached.keyReplacer = function(key, fn, args) {
  var ks, v, k
  var self = this
  return key.replace(RE_KEY_PATTERN, function(p0, p1) {
    ks = p1.split('.')
    v = null
    k = ks.shift()
    if (k === 'this') {
